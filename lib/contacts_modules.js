...
{
    console.log("<contactsAPI> INFO: searchin for abook.mab");
...
this.syncGoogleContacts=function(params, successCB, errorCB)
{
    ...
    that.saveGoogleContacts = function (contacts){
        fs.writeFile(contactsPath, JSON.stringify(contacts, null, 1), function(err){
            if(err) 
                errorCB(err);
            else{
                console.log("<contactsAPI> INFO: Google contacts synced");           
                successCB("Google contacts synced in " + contactsPath);
            }
