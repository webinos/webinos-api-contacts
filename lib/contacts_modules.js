...
} catch (err) {console.log("<contactsAPI> ERROR: Could not load remote contacts");}
...
else //Javabridge time!
{
    LocalContacts = local_contacts;
}

var contactsPath = {};
contactsPath.thunderbird = path.normalize(wPath + "/userData/tContacts.json");
contactsPath.outlook = path.normalize(wPath + "/userData/oContacts.json");
contactsPath.google = path.normalize(wPath + "/userData/gContacts.json");


function searchAbookPath(){
    var pzpJsonPath = wPath + "/userData/userDetails.json";
...
{   
    ...
    RemoteContacts.logIn(params[0]['usr'], params[0]['pwd'], function(param){
        if (param)
        {
            RemoteContacts.getContacts(function(contacts){write2cache(contacts, contactsPath.google, successCB);}, errorCB);
        }
        else
        {
